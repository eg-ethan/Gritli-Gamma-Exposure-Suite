...
class GarchDealerAnalytics:
    ...
    def compute_variance_risk_premium(self, atm_iv: float, forecast_horizon_days: int = 5) -> dict:
        ...
        if vrp > 0.03:
            regime = "HARVESTABLE_VRP (Implied Overpriced / Mean-Reversion Favored)"[cite: 4]
        elif vrp < -0.01:
    ...
    def calculate_dollar_gex(self, chain_df: pd.DataFrame, spot: float) -> pd.DataFrame:
        ...
        """
        df = chain_df.copy()
        # Dollar GEX = DealerInventory * Gamma * S^2 * 0.01 * Multiplier[cite: 2]
        df['dollar_gex'] = (
    ...
    def evaluate_market_state(self, spot: float, chain_df: pd.DataFrame, 
        ...
        call_wall = calls.groupby('strike')['dollar_gex'].sum().idxmax()[cite: 2]
        put_wall = puts.groupby('strike')['dollar_gex'].sum().idxmin()[cite: 2]
        
        vrp_data = self.compute_variance_risk_premium(atm_iv)
        
        # Characterize regime based on dealer exposure and conditional volatility[cite: 1, 3, 5]
        if total_gex > 0:
            gamma_regime = "POSITIVE_GAMMA (Dealers Suppress Realized Vol)"[cite: 1, 3, 5]
            actionable_context = (
                f"Fading extreme moves into Call Wall ({call_wall}) and Put Wall ({put_wall}). "
                "Order flow expected to mean-revert."[cite: 2, 5]
            )
        else:
            gamma_regime = "NEGATIVE_GAMMA (Dealers Accelerate Realized Vol)"[cite: 1, 3, 5]
            actionable_context = (
                "Breakout mode active. Hedging flows reinforce directional momentum. "
                "Avoid fading moves; trade with order flow."[cite: 1, 5]
            )
...
if __name__ == "__main__":
    ...
    for t in range(1, days):
        ...
        # Manual leverage: downside returns induce higher next-day volatility[cite: 4]
    ...
    print(f"Gamma (Leverage Asymmetry): {garch_fit['gamma_leverage']:.4f}")[cite: 4]
    ...
    for k in strikes:
        # Calls: Dealer assumed short (-OI) from retail speculation, or long (+OI)[cite: 1, 3]
        mock_data.append({
            'strike': k,
            'option_type': 'C',
        ...
        # Puts: Customer protective hedging implies dealer short puts (-OI)[cite: 1, 2]
        mock_data.append({
            'strike': k,
            'option_type': 'P',
            'gamma': max(0.001, 0.05 - 0.002 * abs(spot_price - k)),
    ...
    print(f"Aggregate Dollar GEX:   ${state['total_dollar_gex']:,.0f}")[cite: 2]
    print(f"Gamma Regime:           {state['gamma_regime']}")[cite: 1, 3, 5]
    print(f"Call Wall (Resistance): ${state['call_wall']:.2f}")[cite: 2, 5]
    print(f"Put Wall (Support):     ${state['put_wall']:.2f}")[cite: 2]
    ...
    print(f"VRP Regime:             {state['vrp_analysis']['regime']}")[cite: 4]
    print(f"Actionable Context:     {state['actionable_context']}")[cite: 2, 5]